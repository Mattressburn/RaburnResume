...
export default function VinylDashboard() {
  ...
  useEffect(() => {
    loadData().then((rows) => {
      ...
      const manual = manualDurations[key];

      // prefer manual, else original secs
      const secs = manual ? toSecs(manual) : (r.secs ?? null);
 ...
 const labelCounts = useMemo(() => {
  ...
  const filtered = useMemo(() => {
    const ymin = qs.ymin, ymax = qs.ymax, dmax = qs.dmax;
    ...
    return data.filter((d) => {
      ...
      if (labSel.size) {
        ...
        let ok = false;
        labSel.forEach((l) => { if (labs.has(l)) ok = true; });
        if (!ok) return false;
      }
      return true;
    });
  ...
  const yearBuckets = useMemo(() => {
    const m = new Map();
    filtered.forEach((d) => { if (d.year != null) m.set(d.year, (m.get(d.year) || 0) + 1); });
    return Array.from(m.entries()).sort((a, b) => a[0] - b[0]).map(([y, c]) => ({ year: y, count: c }));
  }, [filtered]);
  ...
  const longest = useMemo(() => {
    return filtered.filter((d) => d.secs != null).slice().sort((a, b) => b.secs - a.secs).slice(0, 10);
  }, [filtered]);
  ...
  const LeftTick = ({ x, y, payload }) => (
    <text x={x - 8} y={y} dy={4} textAnchor="end" fill="#a1a1aa" fontSize={12}>
      {payload.value}
    </text>
  );

  const yearsMin = defaults.ymin, yearsMax = defaults.ymax;
...
// key = `${artist}|||${title}` ; value = "mm:ss"
// artist|||title -> seconds
const manualDurations = {
  "The Who|||5:15": 300,
  "ZZ Top|||Waiting for the Bus/Jesus Just Left Chicago": 383, // if your data uses “Waitin’ …”, change key to match
  ...
  "Pink Floyd|||Shine On You Crazy Diamond (Parts I-V)": 813 // optional hyphen variant
};


const toSecs = (v) => {
  if (v == null) return null;
  if (typeof v === "number" && Number.isFinite(v)) return v; // already seconds
  if (typeof v === "string") {
  ...
  if (!ready) return <div className="min-h-screen bg-black text-zinc-100 flex items-center justify-center">Loading…</div>;
      ...
      <div className="mx-auto max-w-7xl px-4 py-6 space-y-6">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <a href="/fan500.json" className="retro-btn">Download JSON</a>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="retro-card p-4">
            ...
            <div className="flex items-center justify-around flex-wrap gap-4">
              ...
              <div className="flex flex-col items-center">
                ...
                <div className="flex items-center gap-1">
                  <button onClick={() => setQS({ ymin: clamp(qs.ymin - 1, defaults.ymin, qs.ymax) })} className="retro-step">-</button>
                  <RetroKnob value={qs.ymin} min={defaults.ymin} max={qs.ymax} onChange={(v) => setQS({ ymin: v })} />
                  <button onClick={() => setQS({ ymin: clamp(qs.ymin + 1, defaults.ymin, qs.ymax) })} className="retro-step">+</button>
                </div>
              ...
              <div className="flex flex-col items-center">
                ...
                <div className="flex items-center gap-1">
                  <button onClick={() => setQS({ ymax: clamp(qs.ymax - 1, qs.ymin, defaults.ymax) })} className="retro-step">-</button>
                  <RetroKnob value={qs.ymax} min={qs.ymin} max={defaults.ymax} onChange={(v) => setQS({ ymax: v })} />
                  <button onClick={() => setQS({ ymax: clamp(qs.ymax + 1, qs.ymin, defaults.ymax) })} className="retro-step">+</button>
                </div>
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {[{ label: "The 60s", range: [1960, 1969] }, { label: "Early 70s", range: [1970, 1974] }, { label: "Late 70s", range: [1975, 1979] }, { label: "Full range", range: [yearsMin, yearsMax] }].map(({ label, range: [a, b] }) => {
                const active = qs.ymin === a && qs.ymax === b;
                return (<button key={label} className={`retro-chip ${active ? "active" : ""}`} onClick={() => setQS({ ymin: a, ymax: b })}>{label}</button>);
              })}
            </div>
          </div>
          <div className="retro-card p-4">
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="text-xs text-zinc-400">
                <span className="inline-block px-2 py-1 rounded-full" style={{ background: "var(--retro-cream)", color: "var(--retro-ink)" }}>≤ {qs.dmax} min</span>
              </div>
            </div>
            <input type="range" min={0} max={31} value={qs.dmax} onChange={(e) => setQS({ dmax: parseInt(e.target.value, 10) })} className="vinyl-range" />
          ...
          {/* LABELS + ARTISTS ROW */}
          <div className="grid grid gap-4 md:col-span-2
          grid-cols-1
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="retro-card p-4"><div className="text-xs" style={{color:"#6a5a37"}}>Tracks</div><div className="text-2xl font-semibold">{filtered.length}</div></div>
          <div className="retro-card p-4"><div className="text-xs" style={{color:"#6a5a37"}}>Artists</div><div className="text-2xl font-semibold">{new Set(filtered.map(d => normalizeArtist(d.artist))).size}</div></div>
          <div className="retro-card p-4"><div className="text-xs" style={{color:"#6a5a37"}}>Total playtime</div><div className="text-2xl font-semibold">{(() => { const s = filtered.reduce((a, b) => a + (b.secs || 0), 0); const h = Math.floor(s / 3600), m = Math.floor((s % 3600) / 60); return `${h}h ${m}m`; })()}</div></div>
          <div className="retro-card p-4"><div className="text-xs" style={{color:"#6a5a37"}}>Avg length</div><div className="text-2xl font-semibold">{(() => { const arr = filtered.filter(d => d.secs != null).map(d => d.secs); if (!arr.length) return '-'; const ave = Math.round(arr.reduce((a, b) => a + b, 0) / arr.length); const mm = Math.floor(ave / 60), ss = String(ave % 60).padStart(2, '0'); return `${mm}:${ss}`; })()}</div></div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          <div className="retro-card p-4 lg:col-span-2">
            ...
            <button className="text-xs px-2 py-1 border border-zinc-800 rounded hover:bg-zinc-800" onClick={() => setQS({ ymin: defaults.ymin, ymax: defaults.ymax })} disabled={qs.ymin === defaults.ymin && qs.ymax === defaults.ymax} title="Clear year filter">Clear</button>
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={yearBuckets} onClick={onYearBarClick}>
                  ...
                  <YAxis stroke="#a1a1aa" />
                  <Tooltip cursor={{ fill: '#ffffff10' }} contentStyle={{ background: '#18181b', border: '1px solid #3f3f46', color: '#f4edd7' }} />
                  <Bar dataKey="count">
                    {yearBuckets.map((_, idx) => (
                      <Cell key={`cell-${idx}`} fill={barPalette[idx % barPalette.length]} />
                    ))}
                  </Bar>
          ...
          <div className="retro-card p-4">
            ...
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={topArtists} layout="vertical" margin={{ left: 40 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="var(--retro-grid)" />
                  <XAxis type="number" stroke="#4b3f2a" />
                  <YAxis dataKey="name" type="category" width={145} interval={0} tickLine={false} axisLine={false} tick={<LeftTick />} stroke="#4b3f2a" />
                  <Tooltip cursor={{ fill: '#00000008' }} contentStyle={{ background: 'var(--retro-paper)', border: '1px solid var(--retro-line)', color: 'var(--retro-ink)' }} formatter={(v, k) => (k === 'count' ? [v, 'Tracks'] : [v, k])} labelFormatter={(label) => label} />
                  <Bar dataKey="count">
        ...
        <div className="retro-card p-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {longest.map((d, i) => (<div key={d.n ?? `${d.artist}|${d.title}|${d.year}|${i}`}
              className="flex gap-3 p-3 rounded-xl"
              style={{background:"var(--retro-paper)", border:"1px solid var(--retro-line)"}}>
              <img src={d.cover || "https://placehold.co/80x80?text=No+Art"} alt="cover" className="w-16 h-16 rounded object-cover" loading="lazy" /><div className="min-w-0"><div className="text-sm font-medium truncate">{d.title}</div><div className="text-xs text-zinc-400 truncate">{d.artist} • {d.album || '—'} • {d.year || '—'}</div><div className="text-xs mt-1">{(() => { if (d.secs == null) return '—'; const mm = Math.floor(d.secs / 60), ss = String(d.secs % 60).padStart(2, '0'); return `${mm}:${ss}`; })()}</div></div></div>))}
          </div>
        </div>

      {/* TRACKS TABLE (Retro / Light) */}
<div className="retro-card p-4">
  <div className="text-sm mb-3 font-medium">Tracks ({filtered.length})</div>

  {/* retro-table = zebra striping, borders, light bg */}
    ...
    <table className="min-w-full text-sm font-mono">
      ...
      <tbody>
        {filtered.map((d, i) => (
          <tr key={i}>
            {/* Track number */}
            <td className="py-2 pr-3 text-[color:var(--retro-ink)]">
              {d.n ?? ""}
            </td>
